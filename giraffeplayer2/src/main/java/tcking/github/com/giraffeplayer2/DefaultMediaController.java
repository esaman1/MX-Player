...
import static tcking.github.com.giraffeplayer2.Util.Utils.Islock;
...
public class DefaultMediaController extends BaseMediaController {
    ...
    protected ImageView imgFloatScreen, app_video_lock, app_video_unlock, imgForward, imgBackward;
    ...
    protected final View.OnClickListener onClickListener = new View.OnClickListener() {
        ...
        public void onClick(View v) {
            ...
            if (v.getId() == R.id.app_video_fullscreen) {
                int aspectRatio = player.getVideoInfo().getAspectRatio();
                int selectedAspectRatio;
                if (aspectRatio == VideoInfo.AR_ASPECT_WRAP_CONTENT) {
                    selectedAspectRatio = VideoInfo.AR_ASPECT_FIT_PARENT;
                    app_video_fullscreen.setImageResource(R.drawable.ic_fit);
                } else if (aspectRatio == VideoInfo.AR_ASPECT_FIT_PARENT) {
                    selectedAspectRatio = VideoInfo.AR_ASPECT_FILL_PARENT;
                    app_video_fullscreen.setImageResource(R.drawable.ic_crop);
                } else {
                    selectedAspectRatio = VideoInfo.AR_ASPECT_WRAP_CONTENT;
                    app_video_fullscreen.setImageResource(R.drawable.ic_fullscreen);
                }
                player.aspectRatio(selectedAspectRatio);
...
//                player.stop();
                imgFloatScreen.setVisibility(View.GONE);
            ...
            } else if (v.getId() == R.id.imgFloatScreen) {
                imgFloatScreen.setVisibility(View.GONE);
    ...
    public void init() {
        ...
        app_video_lock = $.id(R.id.app_video_lock).view();
        app_video_unlock = $.id(R.id.app_video_unlock).view();
        app_video_fullscreen = $.id(R.id.app_video_fullscreen).view();

        imgForward = $.id(R.id.imgForward).view();
        imgBackward = $.id(R.id.imgBackward).view();

        app_video_lock.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Islock = true;
                showBottomControl(false);
                $.id(R.id.app_video_unlock).visibility(View.VISIBLE);
            }
        });

        app_video_unlock.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Islock = false;
                showBottomControl(true);
                $.id(R.id.app_video_unlock).visibility(View.GONE);
            }
        });
        ...
        imgForward.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                Intent localIn = new Intent("FORWARD");
        ...
        imgBackward.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                LocalBroadcastManager lbm = LocalBroadcastManager.getInstance(context);
                Intent localIn = new Intent("BACKWARD");
                lbm.sendBroadcast(localIn);
            }
        });
    }
...
//        $.id(R.id.app_video_fullscreen).visibility(show ? View.VISIBLE : View.GONE);
        if (Islock) {
            ...
            Intent localIn = new Intent("TAG_HIDE");
            lbm.sendBroadcast(localIn);
        } else {
            ...
            if (show) {
                LocalBroadcastManager lbm = LocalBroadcastManager.getInstance(context.getApplicationContext());
                Intent localIn = new Intent("TAG_SHOW");
                lbm.sendBroadcast(localIn);
            } else {
                LocalBroadcastManager lbm = LocalBroadcastManager.getInstance(context.getApplicationContext());
                Intent localIn = new Intent("TAG_HIDE");
...
//            $.id(R.id.app_video_fullscreen).invisible();
    ...
    @Override
    protected void initView(View view, VideoView videoView) {
        init();
        ...
        imgFloatScreen = $.id(R.id.imgFloatScreen).view();
        ...
        $.id(R.id.imgFloatScreen).clicked(onClickListener);
